...
1. **Write.** The customer types items and quantities onto a paper-styled list
   (`mobile/src/components/GroceryListEditor.tsx:34`), or photographs a
   handwritten list and lets the server read it into the same editable lines
   (`mobile/src/components/ScanListPhoto.tsx:24`). They can also browse the
   catalogue and tap "+" on a product card, which adds it to that same list
   (`mobile/src/components/ProductCard.tsx:120`).
2. **Send.** One Send flow, shared everywhere
   (`mobile/src/features/customer/draft-list/use-send-draft.ts:22`). It requires
...
3. **Get price.** The shop prices the list in the admin panel. The customer
   ...
   (`mobile/src/features/customer/grocery-list/types.ts:4`), and can chat with the
   shop about the order (`mobile/src/components/ChatSheet.tsx:67`).
4. **Collect.** They pay over UPI (a deep link into GPay/PhonePe —
   `mobile/src/lib/upi.ts:6`) or at the counter, and collect in person.

Bilingual, Hindi-first: the app defaults to Hindi
(`mobile/src/lib/i18n/index.ts:17`) and asks the language on first launch
(`mobile/src/screens/LanguagePicker.tsx:8`).
...
|---|---|---|
| Components / screens | `PascalCase.tsx`, named export (never `default`) | `mobile/src/screens/ShopScreen.tsx:125` |
...
| Styling | NativeWind `className`; inline `style` only for computed values (shadows, measured sizes) | `mobile/src/components/ProductCard.tsx:72` vs `:102` |
| Colours | Semantic Tailwind tokens (`bg-primary`, `text-muted-foreground`) defined in `mobile/tailwind.config.js:12`. Raw hex appears only where a non-Tailwind API needs it (`@expo/vector-icons` `color`, SVG `fill`) | `mobile/src/components/CustomTabBar.tsx:15` |
| Comments | Explain **why**, not what. Several are load-bearing — see §11 | `mobile/src/components/ProductCard.tsx:79` |
...
Order matters. From `mobile/App.tsx:118`:

```
GestureHandlerRootView                     App.tsx:119   — required by the Sheet's pan gesture
└─ ClerkProvider (tokenCache = SecureStore) App.tsx:120
   └─ SafeAreaProvider                      App.tsx:124
      ├─ NavigationContainer                App.tsx:125
      │  └─ PortalProvider                  App.tsx:131   — INSIDE the container on purpose
      │     ├─ <Bootstrap/>                 App.tsx:132   — renders null; runs the startup effects
      │     ├─ <RootNavigator/>             App.tsx:133
      │     ├─ <GroceryListSheet/>          App.tsx:135   — the list paper
      │     ├─ <QuantitySheetHost/>         App.tsx:137   — the ONE quantity picker (§11)
      │     ├─ <UpdatePrompt/>              App.tsx:138   — OTA
      │     └─ <StoreUpdatePrompt/>         App.tsx:140   — Play Store
      └─ <Toaster/>                         App.tsx:147   — ABOVE the sheets, not inside them
```

Two placements are deliberate and have bitten before:

- `PortalProvider` sits **inside** `NavigationContainer` (`mobile/App.tsx:126-130`)
...
- `Toaster` sits **outside** `PortalProvider` (`mobile/App.tsx:144-147`) so a
...
```mermaid
flowchart TD
    subgraph RootStack["RootNavigator — native stack (navigation/RootNavigator.tsx:12)"]
        direction TB
    ...
    subgraph TabNav["TabNavigator — CustomTabBar, freezeOnBlur (navigation/TabNavigator.tsx:80)"]
        direction LR
    ...
    CTR(["Centre tab button<br/>CustomTabBar.tsx:229"]) -.opens.-> GLS
    GLS["<b>GroceryListSheet</b><br/>portal, not a screen<br/>mounted at App.tsx:135"]
...
| **Lists tab** | Renders `<AuthView>` instead of the list | `mobile/src/screens/MyListsScreen.tsx:433` |
| **Account tab** | Renders `<AuthView>`, with language + terms in the footer | `mobile/src/screens/AccountScreen.tsx:225` |
| Wishlist screen | An empty state, *not* a login | `mobile/src/screens/WishlistScreen.tsx:27` |
| Sending a list | Toast, close the sheet, `navigate("SignIn")` | `mobile/src/features/customer/draft-list/use-send-draft.ts:85` |
| Wishlist heart (card) | Toast "sign in to save" | `mobile/src/components/ProductCard.tsx:46` |
| Wishlist heart (details) | Toast "sign in to save" | `mobile/src/features/customer/products/details/store.ts:104` |

**Two screens are not in any navigator.** `SplashScreen` and `LanguagePicker` are
rendered directly by `App.tsx` (`mobile/App.tsx:107-113`, `:150-154`) before and
over the navigator. The splash is drawn **on top** of a mounted app so Clerk, the
Home request and the customer's lists all load behind it (`mobile/App.tsx:115-117`).
...
(`mobile/src/navigation/types.ts:5-13`; applied at
`mobile/src/screens/ShopScreen.tsx:161-184` and
`mobile/src/screens/MyListsScreen.tsx:415-419`).

`freezeOnBlur: true` (`mobile/src/navigation/TabNavigator.tsx:100`) means an
...
### HomeScreen — `mobile/src/screens/HomeScreen.tsx:29`
...
**Loads.** `loadHome()` on mount (`:36`), then `loadHome({ refresh: true })` on
every focus, plus `loadLists()` when signed in (`:44-52`). The refresh is
rate-limited to once a minute and does not blank the screen
(`mobile/src/features/customer/home/store.ts:24`, `:42`).

**Stores.** `useCustomerHomeStore` (`:33`), `useCustomerGroceryListStore`
(`:43`, for the journey card's data), `useCustomerDisplayName` (`:34`, which
...
single `FlatList` (`:64`, `:200`) so the screen is **one** virtualized list — no
nested scroll views. The search bar is a *button* (`SearchEntry`,
`mobile/src/components/SearchBar.tsx:63`), not a field: it navigates to Shop with
`openSearch: true`, because a field that shows nothing until you find the
keyboard's search key reads as broken.

### ShopScreen — `mobile/src/screens/ShopScreen.tsx:125`
...
(`:131`) — a hook, not a store
(`mobile/src/features/customer/products/use-customer-collections.ts:24`). It
fetches categories once and re-fetches products whenever the filter/sort/search
query changes, with search debounced 300 ms (`:39-42`).

**Stores.** `useDraftListStore` only, for the draft count on the sticky bar (`:186`).

**Renders.** `SearchBar`, sort chips, `RailItem` rail, `ProductCard` grid, and a
sticky "N items in your list — view & send" bar when the draft is non-empty (`:363`).

**Non-obvious.**
- The tab stays mounted, so the hook's *initial* category is read only once.
  Three effects (`:161`, `:170`, `:178`) apply later hand-offs from Home, each
  calling `startFresh()` to drop leftover filters and search, then clearing its
  own param.
- `searchFocusKey` (`:150`, `:250`) is bumped to **remount** the search field so
  it takes focus even when it was already open from an earlier visit.
- `cards` is memoised (`:192`) and `renderCard`/`openProduct` are `useCallback`
  (`:205`, `:210`) — a fresh object literal per render would defeat the memoised
  `ProductCard`.
- `sortOptions` currently has exactly one entry, `recent` (`:34`). The filter
  machinery for brand/colour/size exists in the hook but has no UI.

### MyListsScreen (the "Lists" tab) — `mobile/src/screens/MyListsScreen.tsx:393`

**Purpose.** The customer's sent orders, plus their unsent draft.

**Loads.** `loadLists()` on focus when signed in (`:424`), and pull-to-refresh (`:466`).

**Stores.** `useCustomerGroceryListStore` (`:399-401`), `useDraftListStore` (`:403`).

**Renders.** Status tabs (`active` / `completed` / `cancelled`, `:52`), a
`DraftCard` wrapping the shared `GroceryList` editor on the active tab (`:365`,
`:513`), and a `ListCard` per order (`:140`).

**Non-obvious.**
- `ListCard` marks a list seen only when the tab **is focused**
  (`:143`, `:200-204`). Without `useIsFocused()`, a background tab would silently
  clear the "New update" badge the customer never saw.
- Each card subscribes to *only* its own busy flag (`:149`), because subscribing
...
- "The shop is busy" after 30 minutes is computed purely from the list's age
  (`:49`, `:161-164`) — no backend field, no shopkeeper action.
- Item removal is position-based, so only one removal may be in flight and the
  position is re-read at confirm time, not at alert-open time (`:173-198`).
- Payment buttons appear only when priced **and** still live — a cancelled or
  completed order must never ask for money again (`:321-323`).

### AccountScreen — `mobile/src/screens/AccountScreen.tsx:168`

**Purpose.** Identity, saved details, order counters, settings, sign out.

**Loads.** `loadLists()` + `loadProfile()` on focus (`:189`).

**Stores.** `useCustomerGroceryListStore` (`:177-181`), `useCustomerAccountStore`
(`:183-185`), plus Clerk's `useUser()` (`:173`).

**Renders.** `ProfileAvatar`, an info card, three `StatCard`s, a settings card
(`MenuRow` × N, plus the inline `LanguageRow` at `:38`), and `ProfileEditSheet`.

**Non-obvious.**
- Display values prefer the **saved profile** (what the shop sees on an order)
  and fall back to Clerk (`:254-259`).
- Sign out hands this device's push token back to the server **before** ending
  the session (`:199-202`) — otherwise the next person on a shared phone keeps
  getting the previous customer's order alerts
  (`mobile/src/features/customer/push/registry.ts:3-8`).
- The Help row is hidden unless `EXPO_PUBLIC_SHOP_WHATSAPP` is set (`:392`,
  `mobile/src/lib/env.ts:6`).
- "Member since" shows whole months once there is at least one, else days (`:271-280`).

### ProductDetailsScreen — `mobile/src/screens/ProductDetailsScreen.tsx:44`
...
holds a different product (`:76-81`).

**Stores.** `useCustomerProductDetailsStore` (`:54`), `useCustomerWishlistStore`
(`:52`), `useDraftListStore` (`:67`), `useAuthStore` (`:51`).

**Renders.** `QuantityControl` inline (`:200`), `Badge`, `Button`, and a sticky
action bar (`:318`).

**Non-obvious.** Product pages **stack** — a related product does
`navigation.push` (`:285`) — yet they all share **one** store. Hence the
`productId` guard on focus (`:79`) and inside the store itself
(`mobile/src/features/customer/products/details/store.ts:67`, `:78`). Colours and
sizes are apparel leftovers; they render only when the product has them.

### AuthScreen — `mobile/src/screens/AuthScreen.tsx:10`

A thin wrapper: `<AuthView>` with a close bar, presented as a modal
(`mobile/src/navigation/RootNavigator.tsx:45`), calling `navigation.goBack()`
once signed in.

### WishlistScreen — `mobile/src/screens/WishlistScreen.tsx:15`

Saved products as a plain `ScrollView` list. `loadWishlist()` on mount when
signed in (`:23`). Three states: signed-out message, empty message, list.

### LegalScreen — `mobile/src/screens/LegalScreen.tsx:29`

Static privacy policy + terms, hard-coded in the file. **`SHOP_NAME`,
`CONTACT_EMAIL` and `LAST_UPDATED` at `:9-11` must be edited for a real shop
before publishing.** It documents the photo flow (photo is read and discarded,
`:47-51` of the rendered text) — keep it in step with `ScanListPhoto`.

### SplashScreen — `mobile/src/screens/SplashScreen.tsx:9`

A decorative loading screen driven by a fake progress counter in `App.tsx`
(`mobile/App.tsx:88-105`: +4 % every 40 ms, then a 700 ms hold). It is **not**
tied to real loading — the app behind it is already mounted and fetching.

### LanguagePicker — `mobile/src/screens/LanguagePicker.tsx:8`
...
(`mobile/src/lib/i18n/index.ts:34`).
...
|---|---|---|---|---|
| `useDraftListStore` | `features/customer/draft-list/store.ts:114` | `rows`, `hydrated`, `nextId` — the unsent list | Editor, "Add to list", photo scan, send flow | **Yes** → AsyncStorage key `draft_grocery_list_rows` |
| `useCustomerGroceryListStore` | `features/customer/grocery-list/store.ts:47` | `items`, `unseenCount`, `upi`, `customerPhone`, `loading`, `submitting`, `payingListId` | `loadLists`, `submitList`, `markSeen`, `removeItem`, pay actions | No |
| `useCustomerHomeStore` | `features/customer/home/store.ts:29` | `data` (banners, categories, recentProducts, coupons), `loading` | `loadHome` | No |
| `useCustomerAccountStore` | `features/customer/account/store.ts:20` | `profile` (name, email, phone) | `loadProfile`, `setProfile` after an edit | No |
| `useCustomerWishlistStore` | `features/customer/wishlist/store.ts:30` | `items`, `isOpen` | `loadWishlist`, `toggleItem`, `removeItem`, details store | No |
| `useCustomerProductDetailsStore` | `features/customer/products/details/store.ts:45` | `productId`, `data`, `loading`, selected image/colour/size | `loadProduct`, selection setters | No |
| `useGrocerySheetStore` | `features/customer/grocery-sheet/store.ts:13` | `isOpen` for the list sheet | Centre tab button, banners, journey card, send flow | No |
| `useQuantitySheetStore` | `features/customer/quantity-sheet/store.ts:23` | `target` — which product the root picker is asking about | `ProductCard`'s "+" | No |
| `useAuthStore` | `features/auth/store.ts:18` | `status`, `isBootstrapped`, `user`, `error` | `useBootstrapAuth` | No |
| `useToastStore` | `lib/toast.ts:19` | `toasts[]` | The `toast.*` helpers | No |

`lib/i18n/index.ts` also persists the chosen language to AsyncStorage under
`app_language` (`mobile/src/lib/i18n/index.ts:8`, `:34`) — not a store, but the
...
- **One source of truth for "how many items".** `isSendableRow` (a row with a
  non-empty name, `:39`) and `countSendableRows` (`:43`) back the tab badge, the
  centre-button badge, the Shop sticky bar, the sheet header and Send — so they
  cannot disagree.
- **The paper grows itself.** `withTrailingBlank` (`:48`) keeps exactly one blank
  line at the end, so there is no item limit. `ensureRows(count)` (`:266`) tops it
  up to fill a tall screen; the list sheet measures its viewport and calls it
  (`mobile/src/components/GroceryListSheet.tsx:168-175`).
- **Writes are debounced 500 ms** (`:60`, `:75`) because saving is a native disk
  write and typing a list would do one per keystroke. An `AppState` listener
  flushes immediately when the app leaves the foreground (`:81-83`), so nothing
  is lost.
- **Hydration is picky.** `hydrate()` restores the saved rows **only if at least
  one row is filled** (`:128`); otherwise it starts fresh at 8 rows rather than
  carrying over a wall of blanks from a grown list (`:124-127`).
- **Adding an existing product bumps rather than duplicates.** `addProduct`
  increments a leading integer quantity (`:177-180`) and leaves free text like
  "half kg" alone. `addProductWithQuantity` **sets** the quantity outright
  (`:216`) — that is what the quantity picker and the details screen use.
- **Scanned lines are ordinary lines** (`:287`). A misread word must be fixable
  before the shop sees it, so nothing about the photo is kept.
- Every text write passes through `stripSpecials`
  (`mobile/src/lib/clean-text.ts:10`) — a blocklist, not a `\p{L}` allowlist, so
...
- `grocery-list/store.ts:23` (`loadTicket`) + `:26` (`inFlight`, so bootstrap and
  a focused tab share one request). A failed load keeps the previous items
  (`:72-78`).
- `account/store.ts:18` — specifically so a profile still loading when the
  customer signs out cannot land afterwards and show the next person their name.
- `wishlist/store.ts:13`.

`home/store.ts:26-27` does the same with `lastLoadedAt` + `inFlight`, and
`use-customer-collections.ts:124` uses a local `current` flag for the same reason
...
```
App() mounts
 ├─ effect App.tsx:80  getStoredLanguage() → i18n.changeLanguage(lang)
 │                     · no stored language → show <LanguagePicker/>  (App.tsx:111)
 │                     · until it resolves  → show <SplashScreen/>    (App.tsx:107)
 ├─ effect App.tsx:88  fake splash progress, 0→100 over ~1.0 s + 700 ms hold
 └─ provider tree mounts immediately, splash drawn OVER it  (App.tsx:150)
     └─ <Bootstrap/>  (App.tsx:29, renders null)
         ├─ useBootstrapAuth()      features/auth/useBootstrapAuth.ts:7
         │   ├─ setApiTokenGetter(() => clerk.getToken())   ← wires lib/api.ts
         │   └─ when Clerk is loaded + signed in: POST /auth/sync → useAuthStore
         ├─ usePushNotifications()  features/customer/push/use-push-notifications.ts:10
         │   └─ when signed in and not yet registered: get Expo token → POST /customer/push-token
         ├─ effect App.tsx:42  useDraftListStore.getState().hydrate()   ← unconditional
         └─ effect App.tsx:50  isSignedIn ? loadLists()+loadWishlist()+loadProfile()
...
**The portal.** The sheet renders into `<Portal>` from `@gorhom/portal`
(`:198`), whose host is mounted at the app root (`mobile/App.tsx:131`). That is
...
is what forced each screen to hand-roll its own sheet before (`:45-48`).

**The drag-to-close gesture.** A `Gesture.Pan()` built fresh per detector (`:157`
...
- `onUpdate` clamps at 0 (`:165`) — dragging **up** does nothing, the sheet is
  already as tall as it gets.
- `onEnd` closes if dragged past **110 px** or flicked faster than **900**
  (`:60-61`, `:169-172`); otherwise it springs back (`:176`).
- The backdrop opacity tracks the drag (`:185-187`), so a half-dragged sheet
  shows a half-lit screen.
- A **tall** sheet (`height` prop) attaches the gesture to the grab bar only
  (`:237-240`), because a scroll and a drag would otherwise fight. A **short**
  sheet makes its whole body draggable (`:245`); taps still land, because a pan
  only starts once the finger moves.

**Keyboard handling.** The sheet's `bottom` rides on the keyboard (`:214`), so
nothing inside is ever left underneath it. The listener (`:118-141`) uses
...
bottom (`:219-223`) so the keyboard eats height off the bottom; pinning only the
bottom would push its header — and Send — off the top of the screen.

**Other essentials.**
- Android hardware back closes the top sheet (`:144-151`). Note
  `predictiveBackGestureEnabled: false` in `mobile/app.json:47`.
- It stays mounted until the closing slide finishes (`:91`, `:106-113`), so it
  slides out instead of vanishing.
- `SheetContentGuard` (`:262`) is an error boundary. Before it, an exception
  inside a sheet took the whole app down to a black screen; now the sheet shows
  the message and can be closed.
- It re-exports `SheetTextInput`, `SheetFlatList`, `SheetScrollView` (`:350-354`)
...
### ProductCard — `mobile/src/components/ProductCard.tsx:29`

Memoised (`:162`) and given a **stable** `onPress(id)` handler (`:26`) so a grid
of these doesn't re-render when the search box or draft count changes. Each card
subscribes to only its **own** saved/not-saved answer (`:37`) rather than the
whole wishlist array. It reads `clerk.session` at press time instead of
subscribing to `useAuth()` (`:46`), because `useAuth` re-renders every card each
time Clerk refreshes its token. The "+" opens the **root** quantity picker
through the store (`:121`). The `expo-image` props are load-bearing — see §11.

### GroceryListEditor — `mobile/src/components/GroceryListEditor.tsx:34`

The paper. A view over `useDraftListStore`. Built for typing a whole list without
touching the screen: the keyboard's **Next** key goes item → quantity → next item
(`:156`, `:178`), and the store appends a fresh line as the last one fills.

- `ROW_HEIGHT = 44` and `PAPER_HEADER_HEIGHT = 48` are **exported** (`:11-12`)
...
- Handlers read `useDraftListStore.getState()` rather than the render's `rows`
  (`:48-51`), because typing on the last line appends a new one — the list can
  change before a keypress is handled.
- `compact` (`:83`) shows written lines plus one blank, for the Lists-tab card.
- `autoFocusOnOpen` waits 350 ms for the sheet's slide-in (`:72`) so the keyboard
  doesn't fight the animation.
- The line **number** is pressable and focuses that line (`:136`), so any tap on
  a row puts the cursor there.

`GroceryList` (`mobile/src/components/GroceryList.tsx:11`) is the inline
composition used on the Lists tab: `compact` editor + `ScanListPhoto` +
`SendListButton variant="block"`.

### ScanListPhoto — `mobile/src/components/ScanListPhoto.tsx:24`

One camera icon beside Send. Offers Camera or Gallery via `Alert` (`:113`),
uploads up to `MAX_PHOTOS_PER_SCAN` = 3
(`mobile/src/features/customer/draft-list/store.ts:92`, kept in step with the
server), and writes the result into ordinary editable lines. The photo is never
stored — not on the phone, not on the server, not on the order (`:16-20`).

- Camera needs a permission; the gallery does **not** on modern Android, because
  the system picker hands over only the chosen photo (`:59-61`).
- Android stops showing the permission dialog after one or two refusals, so a
  ...
  tapping a dead button (`:29-44`).
- It always toasts "please check them" on success (`:100-102`): a misread item
  becomes a wrong bill, and this is the one moment fixing it is free.

### SendListButton — `mobile/src/components/SendListButton.tsx:25`
...
call `useSendDraft()` and both own the `PhonePrompt` that flow may open (`:91`).
It dismisses the keyboard first so the customer can see it sending (`:39`).

### Toaster — `mobile/src/components/Toaster.tsx:19`

Renders `useToastStore.toasts` as pill cards under the status bar,
`pointerEvents="none"` (`:27`). Toasts auto-dismiss after 2500 ms
(`mobile/src/lib/toast.ts:25`). Call it from anywhere — including non-React code
— via `toast.success/error/info` (`mobile/src/lib/toast.ts:34`), a drop-in for
the web client's `sonner` API. Mounted outside the portal (see §1).

### Auth components

- **`AuthPanel`** (`mobile/src/components/auth/AuthPanel.tsx:107`) — the login
  itself. See §7.
- **`AuthView`** (`mobile/src/components/auth/AuthView.tsx:26`) — a scrolling page
  around the panel that keeps the field being typed in above the keyboard. It
  **measures how far its own bottom sits above the screen bottom** (`:37-41`) and
  pads by the *overlap* (`:46-47`), not by the keyboard's full height — inside a
  tab, padding the tab bar's height as well would scroll the active field off the
  top. The `footer` is hidden while typing (`:83`).
- **`GoogleAuthButton`** (`mobile/src/components/GoogleAuthButton.tsx:21`) —
  Clerk SSO via `expo-web-browser`, warmed up on Android
  (`mobile/src/lib/use-warm-up-browser.ts:6`).

### Also worth knowing

| Component | File | Note |
|---|---|---|
| `CustomTabBar` | `components/CustomTabBar.tsx:56` | The bar is one SVG path, because a CSS `border-radius` cradle always meets the straight edge at a visible kink (`:51-55`). The corners beside the arc must be painted the page colour or the navigator's backdrop shows through (`:20-23`). |
| `CurvedCaption` | `components/CurvedCaption.tsx:1` | Devanagari cannot go through SVG `TextPath` — it advances one codepoint at a time, so matras detach and conjuncts break. This splits the text into aksharas and places each along the arc (`:22`, used at `CustomTabBar.tsx:214`). |
| `GroceryListSheet` | `components/GroceryListSheet.tsx:48` | The list paper as a sheet. Send pinned in the header; fills the page with blank lines; keeps the focused line visible — **on iOS only** (`:28-35`). |
| `QuantitySheetHost` | `components/QuantitySheet.tsx:99` | The app's single quantity picker. See §11. |
| `QuantityControl` | `components/QuantityControl.tsx:24` | Unit-aware stepper + presets, shared by the sheet and the details screen. Shows the exact string the shop will receive (`:159`). |
| `ListProgressCard` | `components/ListProgressCard.tsx:103` | Home's lead card. Write → Send → Get price → Collect, lit to where the customer actually is (`:26`). Carries no button: the centre tab button already opens the list. |
| `ChatSheet` | `components/ChatSheet.tsx:67` | Per-order chat, polled every 5 s **only while open** (`:21`, `:102-109`). Optimistic send with rollback (`:117-131`). |
| `BannerCarousel` | `components/BannerCarousel.tsx:37` | Autoplaying promo strip. A banner is only tappable if **this build** understands its link type (`:47-58`) — a newer server may send one it has never heard of. Banners whose image fails are dropped, then retried on the next fetch (`:85-95`). Respects Reduce Motion (`:102-109`). |
| `PhonePrompt` | `components/PhonePrompt.tsx:18` | Asked once, the first time a list is sent, with a trust line explaining why. |
| `ProfileEditSheet` | `components/ProfileEditSheet.tsx:23` | The phone is optional; it only has to be *valid* if typed (`:46-51`). |
| `UpdatePrompt` / `StoreUpdatePrompt` | `components/UpdatePrompt.tsx:17` / `StoreUpdatePrompt.tsx:45` | OTA vs. native release. See §10. |
| `ui/Button`, `ui/Badge`, `ui/Card` | `components/ui/` | Small NativeWind primitives over `cn()` (`lib/utils.ts:4`). |
...
**Base URL** is `env.backendUrl` (`:17`), from `EXPO_PUBLIC_BACKEND_URL`
(`mobile/src/lib/env.ts:2`).

**Timeouts.**
- Every request: **20 s** (`:13`, `:19`). A request that never settles shows a
  spinner for ever.
- Getting the Clerk token: **8 s** (`:14`, `:30-35`). Callers can override the
  request timeout per call — the photo read uses 60 s
  (`mobile/src/features/customer/grocery-list/api.ts:22`, `:45`).

**Token injection** (`:22-46`). A module-level `tokenGetter` is installed once by
`useBootstrapAuth` (`mobile/src/features/auth/useBootstrapAuth.ts:12`). The
interceptor races it against an 8-second timer and, on timeout or throw,
**continues without a token** (`:25-38`). This is deliberate: if Clerk is slow or
broken, public screens (Home, Shop, the version check) must still load, so the
code gives up on the *token*, never on the *request*.

**Envelope unwrapping** (`:64-86`). The server always answers
`{ status, data, meta?, errors? }` (`mobile/src/lib/types.ts:16`). `request()`
throws when `status === "error"` **or `data` is falsy** (`:78`), and otherwise
...
the server's first error message, then axios's, then a generic string (`:48-60`,
`:84`). **Consequence:** an endpoint that legitimately returns `null` data would
...
| Module | Calls |
|---|---|
| `features/auth/api.ts:4` | `POST /auth/sync`, `GET /auth/me`. `/auth/sync` returns the same user record `/auth/me` would, so launch needs one round trip (`useBootstrapAuth.ts:30-32`). |
| `features/customer/home/api.ts:4` | `GET /customer/home` |
| `features/customer/products/api.ts:9` | `GET /customer/categories`; `GET /customer/products` with a hand-built, `encodeURIComponent`-escaped query string (`:13-30`); `GET /customer/products/:id` |
| `features/customer/grocery-list/api.ts:11` | `POST /customer/grocery-lists` (response carries `merged` when the server appended to an existing unpriced list); `POST …/read-photo` (multipart, 60 s); `GET /customer/grocery-lists`; `PATCH …/:id/seen`; `PATCH …/:id/pay-at-shop`; `PATCH …/:id/remove-item`; `GET`/`POST …/:id/messages` |
| `features/customer/wishlist/api.ts:7` | `GET /customer/wishlist`, `POST …/items`, `DELETE …/items/:productId` |
| `features/customer/account/api.ts:15` | `GET /customer/profile`, `PATCH /customer/profile` |
| `features/customer/push/api.ts:3` | `POST /customer/push-token`, `DELETE /customer/push-token` (token in the request **body** — `:11`) |
| `components/StoreUpdatePrompt.tsx:51` | `GET /app-version` — called directly with `apiGet`, not through a feature module |
...
(`mobile/src/lib/token-cache.ts:5`), so the session survives restarts. Both
accessors swallow their errors (`:9`, `:17`).

### The flow

```
AuthPanel (components/auth/AuthPanel.tsx:107)
 ├─ "Continue with Google"  → GoogleAuthButton → Clerk useSSO()
 └─ email → ONE field, no password, no separate "sign up":
      signIn.create({ identifier })                    AuthPanel.tsx:245
        ├─ works           → prepareFirstFactor email_code   → mode "signIn"
        └─ form_identifier_not_found                          AuthPanel.tsx:249
                           → signUp.create({ emailAddress })  → mode "signUp"
      → 6-digit code step (CodeBoxes, AuthPanel.tsx:37)
      → signUp only: also collect a name (AuthPanel.tsx:379)
...
customer** (`:31-33`). The code submits itself once six digits are entered,
unless a new customer still owes a name (`:341-351`). Resend has a 30-second
countdown that does **not** start if the resend actually failed (`:325-331`).
...
|---|---|---|
| `clearPending()` | `:28` | If `clerk.session.status === "pending"`, log the blocking task, `signOut()`, return `true` |
| `complete(sessionId, setActive)` | `:43` | `setActive`, then `clearPending`. Returns `"done"` / `"onHold"` (held back and cleared) / `"incomplete"` (Clerk wants something this app doesn't collect) |
| `recoverExisting()` | `:58` | For a `session_exists` failure. Active session → `"signedIn"`; pending → cleared; Clerk says one exists but none is current → full `signOut()` so the next attempt starts clean (`:65`) |

`messageForOutcome` (`:71`) maps a non-`done` outcome to a translation key, so
both screens say the same thing. `clerkErrorCode` (`:7`) digs the code out of
Clerk's `{ errors: [{ code, message }] }` shape.

`AuthPanel` also calls `clearPending()` as soon as the login appears (`:140-142`),
...
|---|---|---|
| Clerk hasn't loaded yet | The API interceptor waits at most 8 s, then sends the request **unauthenticated** and logs a warning — public screens still load | `lib/api.ts:25-38` |
| `useBootstrapAuth` before `isLoaded` | Returns early; nothing is cleared, nothing is fetched | `features/auth/useBootstrapAuth.ts:20` |
| `/auth/sync` fails | `useAuthStore.setError` — `isBootstrapped` still becomes `true`, so the app is not stuck "loading" | `features/auth/store.ts:35` |
| Signed out (or Clerk says so) | `clearAuth()`, and `Bootstrap` clears the lists, wishlist and profile stores | `useBootstrapAuth.ts:22`, `App.tsx:55-59` |
| Clerk not ready when Google is tapped | `authSessionResult` is undefined → toast, nothing attempted | `GoogleAuthButton.tsx:38-41` |
| Customer closes the Google window | Silent no-op (not an error) | `GoogleAuthButton.tsx:44` |
| Clerk's English-only error messages | The common codes are re-worded and translated | `AuthPanel.tsx:174-192` |
...
left on the code step in a hidden tab resets itself to the email step (`:144-151`)
...
Code: `use-send-draft.ts:69` (`send`), `:42` (`doSubmit`), `:120`
(`submitWithPhone`); `grocery-list/store.ts:85` (`submitList`).

**The re-entrancy guard.** `sending` is a `useRef`, not the store's `submitting`
(`use-send-draft.ts:36-38`): `submitting` covers only the POST, but `send()` can
...
**Defaults.** `lng: "hi"` — Hindi-first, rural-first (`mobile/src/lib/i18n/index.ts:17`),
`fallbackLng: "en"`, `compatibilityJSON: "v4"` (`:22`), `returnNull: false`.
The stored choice is applied at launch and the first-launch picker sets it
(`mobile/App.tsx:80-86`).
...
```ts
// mobile/src/lib/i18n/en.ts:38
itemCount_one:   "{{count}} item",
itemCount_other: "{{count}} items",
```

20 such keys exist in each file. Hindi has the same `one`/`other` categories, so
the two suffixes suffice there too (`mobile/src/lib/i18n/hi.ts:41-42`).

**The rule that `hi.ts` must mirror `en.ts` — enforced by a type.** English is the
source of truth:

```ts
export type Translations = typeof en;   // mobile/src/lib/i18n/en.ts:279
```

```ts
export const hi: Translations = { … };  // mobile/src/lib/i18n/hi.ts:5
```
...
4. Use it with `const { t } = useTranslation()` in a component, or the default
   ...
   (`mobile/src/features/customer/grocery-list/store.ts:15`, `:91`).
5. Run `npx tsc --noEmit`.

Note that `stripSpecials` deliberately preserves Devanagari
(`mobile/src/lib/clean-text.ts:5-8`), and the tab caption needs
...
|---|---|---|
| `version` | `1.0.4` | `:5` |
| `runtimeVersion.policy` | `appVersion` | `:10` |
| `updates.url` | `https://u.expo.dev/f5c42a74-…` | `:14` |
| `android.package` / `ios.bundleIdentifier` | `com.skirana.app` | `:48`, `:37` |
| `scheme` | `skirana` (the OAuth redirect target) | `:8` |
| `android.predictiveBackGestureEnabled` | `false` | `:47` |
| `userInterfaceStyle` | `light` | `:9` |
...
2. It makes `Updates.runtimeVersion` equal to the **installed binary's** version —
   ...
   against `GET /app-version` (`mobile/src/components/StoreUpdatePrompt.tsx:21-25`,
   `:84`), which is what makes the whole Play-Store-nag feature shippable over the
...
**The two prompts.**
- `UpdatePrompt` (`mobile/src/components/UpdatePrompt.tsx:17`) — OTA. It re-checks
  on every foreground (`:36-39`) so a freshly published update lands without a
  cold start, and re-asks if a *different* update arrives after "Later" (`:42-48`).
  Inert in Expo Go / dev, where `Updates.isEnabled` is false.
  *(The comment at `:12` states `checkAutomatically: ON_LOAD`; that is
  `expo-updates`' default — it is **not** configured in `app.json`. Unverified
  against the installed module.)*
- `StoreUpdatePrompt` (`mobile/src/components/StoreUpdatePrompt.tsx:45`) — a new
  Play Store build. Android only (`:60`, `:81`). Below the server's `minVersion`
  the update is **mandatory** — no "Later" (`:89-92`).
...
`expo export` will via `@expo/env` (`:9-10`), and **exits non-zero unless
`EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` starts with `pk_live_`** (`:15-29`). Its error
...
unset by default (`mobile/src/lib/env.ts:4-6`). Only `EXPO_PUBLIC_*` variables
reach the bundle.
...
|---|---|---|---|
| `development` | — | internal | `developmentClient: true`, Android APK (`:7-13`) |
| `preview` | `preview` | internal | Android APK (`:14-20`) |
| `production` | `production` | store | `autoIncrement: true` (`:21-24`) |

`cli.appVersionSource: "remote"` (`:4`) — EAS owns the build number. `npm run ota`
...
`mobile/src/components/ui/Sheet.tsx:126-133` and
`mobile/src/lib/use-keyboard-height.ts:6-9`.
...
on iOS (`Sheet.tsx:120-123`). This is why the list sheet shed ~120 lines of
hand-rolled keyboard code (`mobile/src/components/GroceryListSheet.tsx:42-47`).

### 2. `expo-image` `transition` blanks images on Android

`mobile/src/components/ProductCard.tsx:79-83`.
...
(`HomeScreen.tsx:144`, `ShopScreen.tsx:65`, `ProductDetailsScreen.tsx:129`) —
if you ever put those into a `FlatList` cell, drop the prop.

Its companion on the same element: `recyclingKey={product.id}`
(`ProductCard.tsx:91`), so a recycled cell can never flash the previous product's
photo.

### 3. `expo-image` `cachePolicy` defaults to `disk` alone

`mobile/src/components/ProductCard.tsx:84-88`.
...
`mobile/scripts/preflight-ota.cjs:1-6`, guard at `:15`.
...
`mobile/src/features/customer/quantity-sheet/store.ts:3-9`, host at
`mobile/src/components/QuantitySheet.tsx:99`, mounted at `mobile/App.tsx:137`.
...
(`ProductCard.tsx:121`). Never put a `<Sheet>` inside a list cell. The host keeps
the last product's title while the sheet slides away, so it doesn't blank
mid-animation (`QuantitySheet.tsx:111-113`).

### 6. `@gorhom/bottom-sheet` does not work on Reanimated 4

`mobile/src/components/ui/Sheet.tsx:41-43`. Its sheets never open on Expo SDK 54.
That is why `Sheet.tsx` exists. See §5.

### 7. `freezeOnBlur` keeps tabs mounted, so background screens still run effects

`mobile/src/navigation/TabNavigator.tsx:98-100`. Three consequences already fixed:

- `MyListsScreen.tsx:143`, `:200` — without `useIsFocused()`, a background tab
  marks lists "seen" and clears a badge the customer never saw.
- `AuthPanel.tsx:144-151` — a hidden copy on the code step hijacks the device's
  single Clerk sign-in attempt.
- `AuthPanel.tsx:163-170` — a hidden copy would swallow the Android back press
  meant for the visible screen.

### 8. Close the sheet *before* navigating on Android

`mobile/src/features/customer/draft-list/use-send-draft.ts:87-92`.
...
`mobile/src/components/GroceryListSheet.tsx:28-35`.
...
`scrollEventThrottle` / `onRowFocus` are all wired **only** on iOS (`:154-161`,
`:190-197`).

### 10. Do not gate UPI on `Linking.canOpenURL()`

`mobile/src/lib/upi.ts:28-32`.
...
`mobile/src/components/ui/Sheet.tsx:256-259`. The known cause was sheet contents
calling `useNavigation()` while the portal host sat outside `NavigationContainer`
— which is why `PortalProvider` is inside it (`mobile/App.tsx:126-130`). The
`SheetContentGuard` boundary now shows a message instead.

### 12. Hand the push token back before signing out

`mobile/src/features/customer/push/registry.ts:3-8`, called at
`mobile/src/screens/AccountScreen.tsx:199-202`. The server needs the *customer's*
...
`mobile/src/components/ProductCard.tsx:68-71`. `flex-1` collapsed the card to zero
...
`mobile/src/components/CustomTabBar.tsx:39-43`,
`mobile/src/components/CurvedCaption.tsx:4-14`. It advances one codepoint at a
...
`mobile/src/features/customer/grocery-list/store.ts:72-78` (and the same rule in
`home/store.ts:51-53`, `wishlist/store.ts:45-47`, `account/store.ts:27-29`).
