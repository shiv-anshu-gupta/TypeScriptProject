...
- **`client/src/pages/admin/Orders.tsx`** — a complete legacy-order table with
 ...
 still mounted (`server/src/server.ts`, `server/src/routes/admin/orders.routes.ts,69`),
...
(`client/vite.config.ts`); it explicitly exempts `/` and `/index.html` and anything with a file extension or a Vite internal prefix.
...
flowchart TD
 ...
 R -->|"/ or unknown"| Redir["Navigate → /admin<br/>router.tsx,97"]
 Redir --> PL
...
(`client/src/components/admin/common/sidebar.tsx,77-89`), a slide-over
...
- **"Payment received?" amount matcher** — the shopkeeper types the rupee amount
 ...
 at `use-admin-grocery-lists.ts`, helper count at).
...
(labels at), a payment badge shown only once priced, and **Share**.
...
- A **calculator popover** per row
 ...
 → 270" (`price-calculator.tsx`, seeding at).

Drafts live **only in the hook**, keyed by list id
(`use-admin-grocery-lists.ts,42-44`), seeded from whatever the server
...
(`grocery-list-card.tsx,328,386-405`). State is a `Set<number>` stored
...
(`grocery-list-card.tsx,310-324,417-435`). Translation is
...
**Status flow buttons** (`grocery-list-card.tsx,574-621`):
 ...
 **"Update prices"** after (; `isPriced = totalAmount > 0` at).
...
(`grocery-list-chat.tsx,57-63`). Sending clears the input optimistically and
...
(`use-admin-grocery-lists.ts,225,257,271,282,301`).
...
- API: `GET/POST /admin/promos`, `PATCH /admin/promos/:id`,
 ...
 Every mutation returns the full list and replaces state (`use-admin-promo.ts,81`).
...
- **Status computation** is local and mirrors what the app will show:
 ...
 (`client/src/pages/admin/Settings.tsx,39-45`).
...
- The carousel `limit` comes from the server on every response
 ...
 client fallback (`use-admin-banners.ts,31-33`). **The server does not
...
with its object URLs (`banner-uploader.tsx,70-74`).
...
- **The "orders" here are grocery lists, not legacy `Order` documents**
 (`server/src/routes/admin/dashboard.routes.ts,33-39`). Specifically
 ...
 **IST calendar day** server-side
 (`server/src/routes/admin/dashboard.routes.ts,83-104`).
...
orders can be advanced (`Orders.tsx,139-167`), backed by
...
still answers (`server/src/routes/admin/orders.routes.ts,69`, mounted at
...
(`use-admin-grocery-lists.ts,59-66`) — it calls `notifyNewOrders`, which:
...
(`server/src/utils/push.ts,27-75`).
...
|---|---|---|
| Product / category images | 1 MB per file after compression (`client/src/lib/image.ts`) | **No per-file byte cap and no MIME filter** — multer is configured with `fieldSize` (the *text-field* limit), not `fileSizeserver/src/routes/admin/product.routes.ts`. Only `files: 10` actually constrains uploads. |
| Banners | 5 MB, 10 files, 3 MIME types (`banner-uploader.tsx`) | Matches exactly: `MAX_FILE_BYTES = 5 * 1024 * 1024`, `MAX_FILES = 10`, `ALLOWED_TYPES` — `server/src/routes/admin/settings.routes.ts`; multer errors mapped to 400 at |
...
(`server/src/routes/admin/settings.routes.ts,52-55`).
...
sequenceDiagram
 ...
 A->>S: "GET /admin/grocery-lists<br/>use-admin-grocery-lists.ts,77"
...
(`docs/PRODUCTION-SETUP.md:82`, `client/src/lib/env.ts`,
`client/src/main.tsx`).
...
`server/src/routes/admin/product.routes.ts` configures multer with
`fieldSize`, not `fileSize`, and no `fileFilter`. The 1 MB limit and the
`accept="image/*"` attribute exist only in the client
(`client/src/lib/image.ts`,
`client/src/features/admin/products/use-product-form.ts`). Banners are
the correctly-gated path
(`server/src/routes/admin/settings.routes.ts`); products are not. If you
...
falls back to the original text silently and the in-memory cache dies on reload. The "Show हिंदी + English" toggle therefore degrades to
