...
The port is `process.env.PORT || 5000` (`server/src/server.ts`). There is no
`/api` prefix and no version prefix — routers mount straight onto `/auth`,
`/customer` and `/admin` (`server/src/server.ts`).
...
|---|---|---|
| `AppError(statusCode, message)` (`utils/AppError.ts`) | `err.statusCode` | `{ message, code: "APP_ERROR" }` |
| Anything else (Mongoose `CastError`, `ValidationError`, duplicate key, body-parser 413, a bug) | `500` | `{ message: "Internal server error", code: "INTERNAL" }` — the real error is only in the server log (`errorhandler.ts`) |
| No route matched (`middleware/notFound.ts`) | `404` | `{ message: "Route not found GET" }` — the **method**, not the path, and no `code` |

Status codes actually produced by route code: **200**, **201**, **400**, **401**,
**403**, **404**, **409**, **429**, **500**, **503**. There is no 422 and no 502.

Async route handlers are wrapped in `asyncHandler` (`utils/asyncHandler.ts`),
...
| customer | `requireAuth` (`auth.ts`) | 401 `"User is not logged in. Means unauth user! !"` when Clerk gives no `userId`. Applied **router-wide** via `router.use(requireAuth)` on every customer router that needs it. |
| admin | `requireAdmin` (`auth.ts`) | Resolves the DB user, then 403 `"Admin access only"` if `role !== "admin"`. An unauthenticated request hits the 401 from `getDbUserFromReq` first. Applied router-wide on every admin router. |
...
Every authenticated handler calls `getDbUserFromReq(req)` (`auth.ts`):
...
|---|---|---|---|---|
| GET | `/health` | none | Liveness probe. | `server.ts` |
| GET | `/app-version` | none | Latest / minimum Play Store version for the update prompt. | `server.ts` |

### Auth

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| POST | `/auth/sync` | customer | Create, re-link or refresh the DB user after login. | `routes/auth/auth.routes.ts` |
| GET | `/auth/me` | customer | The caller's own user record. | `routes/auth/auth.routes.ts` |

### Customer — catalogue (public)

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| GET | `/customer/home` | none | Home screen payload: banners, categories, 4 newest products, 4 live coupons. | `routes/customer/home.routes.ts` |
| GET | `/customer/categories` | none | All categories, A→Z. | `routes/customer/product.routes.ts` |
| GET | `/customer/products` | none | Active products, filtered and searched. | `routes/customer/product.routes.ts` |
| GET | `/customer/products/:id` | none | One active product plus up to 4 related. | `routes/customer/product.routes.ts` |

### Customer — grocery lists

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| POST | `/customer/grocery-lists/read-photo` | customer | Read photos of a handwritten list into items (Gemini). Stores nothing. | `routes/customer/grocery-list.routes.ts` |
| POST | `/customer/grocery-lists` | customer | Send a list; merges into an unpriced list from the last 6 h. | |
| GET | `/customer/grocery-lists` | customer | All of my lists, newest first, plus badge count and shop UPI details. | |
| PATCH | `/customer/grocery-lists/:listId/seen` | customer | Clear the badge for one list. | |
| PATCH | `/customer/grocery-lists/:listId/remove-item` | customer | Remove one item before packing starts. | |
| PATCH | `/customer/grocery-lists/:listId/pay-at-shop` | customer | Choose to pay at the counter. | |
| POST | `/customer/grocery-lists/:listId/pay-online` | customer | Create a Razorpay order for the list. | |
| POST | `/customer/grocery-lists/:listId/confirm-payment` | customer | Verify the Razorpay signature and mark paid. | |
| GET | `/customer/grocery-lists/:listId/messages` | customer | Read the chat on one of my lists. | |
| POST | `/customer/grocery-lists/:listId/messages` | customer | Send a chat message to the shop. | |

### Customer — profile, addresses, devices

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| GET | `/customer/profile` | customer | My name, email, phone. | `routes/customer/profile.routes.ts` |
| PATCH | `/customer/profile` | customer | Update name and/or mobile; pushes them onto open lists. | |
| GET | `/customer/addresses` | customer | My addresses, default first. | `routes/customer/address.routes.ts` |
| POST | `/customer/addresses` | customer | Add an address. | |
| PATCH | `/customer/addresses/:addressId` | customer | Replace one address's fields. | |
| DELETE | `/customer/addresses/:addressId` | customer | Delete an address. | |
| POST | `/customer/push-token` | customer | Register this device for Expo push. | `routes/customer/push-token.routes.ts` |
| DELETE | `/customer/push-token` | customer | Unregister on sign-out. | |

### Customer — cart, wishlist, promo, checkout, orders

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| GET | `/customer/cart` | customer | My cart. | `routes/customer/cart-wishlist.routes.ts` |
| POST | `/customer/cart/items` | customer | Add a product (with variant) to the cart. | |
| PATCH | `/customer/cart/items/:productId/increase` | customer | +1, capped by stock. | |
| PATCH | `/customer/cart/items/:productId/decrease` | customer | −1, removing the row at 0. | |
| DELETE | `/customer/cart/items/:productId` | customer | Remove one cart row. | |
| POST | `/customer/cart/sync` | customer | Merge a guest cart into the server cart. | |
| GET | `/customer/wishlist` | customer | My wishlist. | |
| POST | `/customer/wishlist/items` | customer | Add a product to the wishlist. | |
| DELETE | `/customer/wishlist/items/:productId` | customer | Remove from the wishlist. | |
| POST | `/customer/promos/apply` | customer | Validate a promo code against an order value. | `routes/customer/promo.routes.ts` |
| POST | `/customer/checkout/create-session` | customer | Price the cart, create an Order + Razorpay order. | `routes/customer/checkout.routes.ts` |
| POST | `/customer/checkout/confirm` | customer | Verify signature, decrement stock, empty the cart. | |
| GET | `/customer/checkout/points` | customer | My points balance. | `routes/customer/checkout-with-points.routes.ts` |
| POST | `/customer/checkout/pay-with-points` | customer | Pay for the cart entirely with points. | |
| GET | `/customer/orders` | customer | My orders (the Order collection, not grocery lists). | `routes/customer/orders.routes.ts` |
| PATCH | `/customer/orders/:orderId/return` | customer | Return a delivered order within 7 days. | |

### Admin — grocery lists and chat

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| GET | `/admin/grocery-lists` | admin | Every list, most recently active first. | `routes/admin/grocery-list.routes.ts` |
| PATCH | `/admin/grocery-lists/:listId/prices` | admin | Price every line; status → `priced`. | |
| PATCH | `/admin/grocery-lists/:listId/status` | admin | Move a list along `packing → … → completed`, or cancel. | |
| PATCH | `/admin/grocery-lists/:listId/mark-paid` | admin | Confirm a counter/UPI payment. | |
| PATCH | `/admin/grocery-lists/:listId/items/:index/availability` | admin | Mark one line in/out of stock. | |
| PATCH | `/admin/grocery-lists/:listId/items/:index` | admin | Edit one line's name/quantity. | |
| POST | `/admin/grocery-lists/:listId/items` | admin | Add a line to an open list. | |
| GET | `/admin/grocery-lists/conversations` | admin | Last message per list, newest first, max 100. | |
| GET | `/admin/grocery-lists/:listId/messages` | admin | Read one list's chat. | |
| POST | `/admin/grocery-lists/:listId/messages` | admin | Reply to the customer. | |

### Admin — catalogue

| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| GET | `/admin/categories` | admin | All categories. | `routes/admin/product.routes.ts` |
| POST | `/admin/categories` | admin | Create a category (optional image). | |
| PUT | `/admin/categories/:id` | admin | Rename / re-image a category. | |
| DELETE | `/admin/categories/:id` | admin | Delete an empty category. | |
| GET | `/admin/products` | admin | All products (any status), optional title search. | |
| GET | `/admin/products/:id` | admin | One product. | |
| POST | `/admin/products` | admin | Create a product with 1–10 images. | |
| PUT | `/admin/products/:id` | admin | Update a product; add/remove/re-cover images. | |
...
| Method | Path | Auth | Purpose | Code |
|---|---|---|---|---|
| GET | `/admin/settings/banners` | admin | Every banner in carousel order. | `routes/admin/settings.routes.ts` |
| POST | `/admin/settings/banners` | admin | Upload 1–10 images as banners. | |
| PUT | `/admin/settings/banners/order` | admin | Reorder the whole carousel. | |
| PATCH | `/admin/settings/banners/:bannerId` | admin | Edit title, visibility, tap action, schedule. | |
| DELETE | `/admin/settings/banners/:bannerId` | admin | Delete a banner and its Cloudinary image. | |
| GET | `/admin/promos` | admin | All promo codes. | `routes/admin/promo.routes.ts` |
| POST | `/admin/promos` | admin | Create a promo code. | |
| PATCH | `/admin/promos/:promoId` | admin | Update a promo code. | |
| DELETE | `/admin/promos/:promoId` | admin | Delete a promo code. | |
| GET | `/admin/orders` | admin | All Orders. | `routes/admin/orders.routes.ts` |
| PATCH | `/admin/orders/:orderId/status` | admin | Change an Order's status. | |
| GET | `/admin/dashboard/lite` | admin | Headline counters. | `routes/admin/dashboard.routes.ts` |
| GET | `/admin/dashboard/daily` | admin | Last 7 IST days: orders and sales. | |
| POST | `/admin/push-token` | admin | Register an admin browser for FCM web push. | `routes/admin/push-token.routes.ts` |
| DELETE | `/admin/push-token` | admin | Unregister an admin browser. | |
...
#### `POST /auth/sync`
`server/src/routes/auth/auth.routes.ts`
...
Mapper: `toPayload` (`auth.routes.ts`). `email` and `name` are omitted when
absent on the record.

Errors: **401**; **409** `"This email is already used by another sKirana account.
Please contact the shop."` (`user-sync.ts`); **500** if Clerk is unreachable.
...
(`user-sync.ts`). Calls the **Clerk Backend API** (`users.getUser`).
...
Note the product key is **`createAt`** (missing "e") — `home.routes.ts`. It is
a mapper key, not a DB field; the DB field is `createdAt`.

Image URLs are rewritten by `cdnImage` (`utils/cloudinary.ts`) to
...
`server/src/routes/customer/product.routes.ts`
...
Query parameters (all optional, all trimmed —):
...
| `search` | string | case-insensitive `title` regex, escaped by `escapeRegex` (`utils/regex.ts`) so `(`, `*`, `+` are literal |
| `sort` | `recent`/`price-low`/`price-high` | **accepted in the type but ignored** — `sortOption` is hard-coded to `{ createdAt: -1 }` |

`status: "active"` is always applied. No pagination, no limit.
...
Mapper: `sizedProduct(product, "card")` (`utils/productImages.ts`) — the whole
...
the same category, newest first.
...
Router guard: `requireAuth` on every route (`grocery-list.routes.ts`).

Shared mapper `mapGroceryList` — used by every list endpoint below:
...
`server/src/routes/customer/grocery-list.routes.ts` — **multipart**, see § 4.1.
...
(`services/photo-list-parser.ts`) — so an unreadable photo returns
...
|---|---|---|
| 400 | `Send a JPG, PNG or WebP photo` | `grocery-list.routes.ts` (fileFilter) |
| 400 | `Each photo must be under 6 MB` | (multer `LIMIT_FILE_SIZE`) |
| 400 | `Send at most 3 photos at a time` | (any other MulterError) |
| 400 | `Choose at least one photo` | |
| 401 | not signed in | `auth.ts` |
| 429 | `Your photo is still being read — one moment.` | `photo-list-parser.ts` (same customer, request in flight) |
| 429 | `Just a moment before the next photo.` | (< 5 s since that customer's last read **finished**) |
| 503 | `Reading photos isn't switched on yet. Please type the items instead.` | (`GEMINI_API_KEY` unset) |
| 503 | `A lot of lists are being read right now. Try again in a minute, or type the items.` | (≥ 12 calls in the current minute, per instance) |
| 503 | `Could not reach the photo-reading service. Check the internet and try again.` | (fetch failed or the 45 s abort fired) |
| 503 | `The photo reader is busy right now. Try again in a minute, or type the items.` | (Gemini returned 429) |
| 503 | `The photo could not be read just now. Try again, or type the items.` | (Gemini non-2xx) and (output failed zod validation) |
...
for the request and are gone with the response (`grocery-list.routes.ts`).
One log line per successful read (`photo-list-parser.ts`).
...
Errors: **400** `Too many items in one request` (`sanitizeItem.ts`); **400**
`A list can have at most 50 items per send`; **400** `Add at least one
item` (`grocery-list.routes.ts`); **400** `This list already has too many
items (max 100).`; **401**.
...
`notifyAdmins` (`utils/webPush.ts`) — `"New grocery list"` or `"List updated"`.
**Telegram** message to every configured chat id (`utils/telegram.ts`). Both are
...
Guards, in order: valid index → `paymentStatus !== "paid"` → status
...
`utils/razorpay.ts`), receipt `GroceryList_<id>`, then stores
...
`utils/razorpay.ts` throws at module load otherwise.
...
(TTL index, `models/Message.ts`), so an old order's chat comes back empty.
...
Body — both fields optional; only the keys present are touched:

| Field | Rules |
|---|---|
| `name` | `cleanField(value, 50, true)` — same allowlist as grocery items, capped at **50** characters (`profile.routes.ts`). Empty after cleaning → 400. |
...
whose status is not `completed` or `cancelled`. Finished and cancelled
lists deliberately keep their historical snapshot.

#### `GET /customer/addresses`
`server/src/routes/customer/address.routes.ts`
...
Sorted default-first. Mapper: `mapAddress`.
...
`false` is ignored (the flag is never cleared here —).
...
`server/src/routes/customer/push-token.routes.ts` and
...
(`utils/push.ts`), which filters to strings starting `ExponentPushToken[` or
`ExpoPushToken[` — any other token is stored but never sent to.
...
Creates the cart if absent. Errors: **400** `Product id is required`;
...
`itemIndex > 0` instead of `>= 0`, so re-adding the **first** row pushes
...
#### `PATCH /customer/cart/items/:productId/decrease` —
Same parameters. Subtracts 1 and splices the row out at 0.
...
silently; quantities are clamped to stock.
...
`server/src/routes/customer/promo.routes.ts`
...
`server/src/routes/customer/checkout.routes.ts`
...
`salePercentage`, validates the promo, creates a Razorpay order and an
...
Side effects: per-item conditional `$inc` on `products.stock`, promo
...
decremented and the order stays `pending`.

#### `GET /customer/checkout/points`
`server/src/routes/customer/checkout-with-points.routes.ts`
...
enough points for this order`; **401**.

Side effects: conditional `$inc` deducting points, stock decrements,
...
deduction the points are credited back in a `catch` — but stock
...
`totalAmount`**, the order marked `returned`. No idempotency guard beyond
...
#### `PATCH /admin/grocery-lists/:listId/prices` —
...
| Field | Rules |
|---|---|
| `items` | array, non-empty, and its **length must equal** the list's current item count |
| `items[].price` | number ≥ 0; rounded with `Math.round`; forced to `0` for an item already marked unavailable |
| `items[].rate` | optional number; kept only if finite and `> 0`, else `0` |
...
(`utils/push.ts`). Awaited, never throws.

#### `PATCH /admin/grocery-lists/:listId/status` —

Path parameter `listId`. Body: `{ "status": "packing" }`.

Allowed values: `packing`, `packed`, `ready`, `completed`, `cancelled`.
`received` and `priced` are **not** settable here — pricing has its own endpoint,
and there is no way back to `received`.

An unpriced list (`totalAmount < 1`) can only be moved to `cancelled`.
...
with the per-status wording from `statusNotification`, e.g.
`"Your order is ready — come and collect it!"`.

#### `PATCH /admin/grocery-lists/:listId/mark-paid` —
...
`at_shop` to `upi`. An already-paid list returns **200** unchanged.
...
#### `PATCH /admin/grocery-lists/:listId/items/:index/availability` —
...
#### `PATCH /admin/grocery-lists/:listId/items/:index` —

Path parameters: `listId`, `index`. Body: `{ "name": "...", "quantity": "..." }` —
each optional; an omitted field keeps the existing value. Both are
...
#### `POST /admin/grocery-lists/:listId/items` —
...
#### `GET /admin/grocery-lists/conversations` —

No parameters. An aggregation over `messages`: newest message per list, sorted by
that message's time, **capped at 100 conversations**. Lists that no
...
`GET /admin/grocery-lists/:listId/messages` and there is no
`GET /admin/grocery-lists/:listId`, so `conversations` is never swallowed by a
parameter route.

#### `GET /admin/grocery-lists/:listId/messages` —
...
#### `POST /admin/grocery-lists/:listId/messages` —
...
Router guard: `requireAdmin` (`admin/product.routes.ts`). Multipart details in
§ 4.2 and § 4.3.
...
(`utils/cloudinary.ts`); one DB insert.
...
Errors: **404** `Product not found` ( — via `requireText`, which happens to
...
NOT passed through `sizedProduct`** — so image URLs come back at full
...
| Field | Rules |
|---|---|
| `existingImages` | JSON string — the array of images the client **kept**. Only `publicId` is read. Absent → keep everything currently stored; unparseable → treated as `[]`, which removes **all** existing images |
...
( explains only that carts/wishlists null-guard the missing reference).
...
Router guard: `requireAdmin` (`admin/settings.routes.ts`). Multipart details
in § 4.4.
...
| `link` | object `{ type, targetId? }`. `type` ∈ `none`, `writeList`, `shop`, `category`, `product` (`models/Banner.ts`). For `category`/`product` a valid, **existing** `targetId` is required |
...
#### `GET /admin/promos` — `routes/admin/promo.routes.ts`
...
#### `POST /admin/promos` — · `PATCH /admin/promos/:promoId` —

Both use `parsePromoPayload`:
...
and 10` (message says 10, the check is 1–100 —); **400** `Promo count must
be atleast 1`; **400** `Promo count must be atleast 0 or more` (this is the
`minimumOrderValue` message —); **400** `Valid start time is required`;
...
#### `GET /admin/orders` — `routes/admin/orders.routes.ts`
...
Side effects: moving to `returned` restores stock for every item;
...
#### `GET /admin/dashboard/lite` — `routes/admin/dashboard.routes.ts`
...
`routes/admin/push-token.routes.ts` and
...
by `notifyAdmins` (`utils/webPush.ts`).
...
Gemini request (`services/photo-list-parser.ts`), then discarded when the
handler returns. It is never written to disk, never uploaded to Cloudinary, and no
reference to it is stored on the list — see the note at `grocery-list.routes.ts`
and `models/GroceryList.ts`. Multiple photos of the same list go in **one**
...
|---|---|---|
| Same customer, read already in flight | `readingNow` set | **429** `Your photo is still being read — one moment.` |
| Same customer, gap since the previous read **finished** | `USER_GAP_MS` = **5 s** | **429** `Just a moment before the next photo.` |
...
have elapsed by the time the customer could tap again.
...
code says so itself: treat it as a brake, never as a security boundary.
`finishedAtByUser` is pruned once it exceeds 500 entries.

Model: `GEMINI_MODEL` or `gemini-3.6-flash`, called over plain REST with
`x-goog-api-key` — no SDK. Output is constrained by a `responseSchema` and then
re-validated with zod; anything unparseable becomes a 503 rather than
...
| **Razorpay** | online payment endpoints | `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET` | **The module throws at import if either is missing** (`utils/razorpay.ts`), so the whole server fails to boot — not just the payment routes |
