...
// so these are the single source for both the row styling and that maths -
// change a height here and the scrolling stays correct.
...
type GroceryListEditorProps = {
  ...
  // Put the cursor on the first line when the list is still empty, so tapping
  // "Write list" lets the customer start typing straight away.
...
// last one fills. The paper is the FIRST child on purpose - the sheet's
// scroll-into-view maths assumes it starts at the top of the scroll content.
export function GroceryListEditor({
  ...
  // "Add item": go to the first empty line. The store always keeps one blank
  // line at the end, so there is always somewhere to go.
  ...
  useEffect(() => {
    ...
    const hasItems = useDraftListStore
      .getState()
      .rows.some((row) => row.name.trim());
    if (hasItems) return; // an existing list is more likely being reviewed
    // Wait for the sheet's slide-in so the keyboard doesn't fight it.
    const timer = setTimeout(() => focusName(0), 350);
  ...
  return (
    <View className="gap-3">
      ...
      {/* Makes "the list has no limit" obvious instead of something only the
          shop owner knows from testing. */}
      <Pressable
        onPress={focusFirstEmpty}
        accessibilityRole="button"
        className="mx-3 flex-row items-center justify-center gap-2 rounded-xl border border-dashed border-primary/40 bg-card py-3 active:opacity-70"
      >
        <Feather name="plus" size={16} color="#3c5a64" />
